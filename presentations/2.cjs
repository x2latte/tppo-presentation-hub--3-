...
async function createP2(outDir) {
  ...
  // Slide 2: Пользователь vs Разработчик
  {
    const s = cs(pres, "Атрибуты качества: пользователь vs разработчик");
    card(s, pres, MX, CY, 4.4, CH, C.ICE);
    s.addText("Что видит конечный пользователь?", { x: MX + 0.18, y: CY + 0.12, w: 4.04, h: 0.40,
      color: C.NAVY, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText([
      { text: "Простоту и отзывчивость интерфейса (Usability).", options: { bullet: true, breakLine: true } },
      { text: "Высокую скорость отклика системы (Performance).", options: { bullet: true, breakLine: true } },
      { text: "Постоянную доступность без сбоев (Uptime / Reliability).", options: { bullet: true } },
    ], { x: MX + 0.18, y: CY + 0.62, w: 4.04, h: 3.2,
      color: C.DARK, fontSize: 11.5, fontFace: "Calibri" });

    card(s, pres, MX + 4.7, CY, 4.46, CH, C.GRNL);
    s.addText("Что важно инженеру и архитектору?", { x: MX + 4.88, y: CY + 0.12, w: 4.1, h: 0.40,
      color: C.NAVY, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText([
      { text: "Модульность: легкое разделение на изолированные блоки.", options: { bullet: true, breakLine: true } },
      { text: "Тестируемость: простое написание автотестов.", options: { bullet: true, breakLine: true } },
      { text: "Поддерживаемость и расширяемость кодовой базы.", options: { bullet: true } },
    ], { x: MX + 4.88, y: CY + 0.62, w: 4.1, h: 3.2,
      color: C.DARK, fontSize: 11.5, fontFace: "Calibri" });
  }

  // Slide 3: Ключевые атрибуты качества
  {
  ...
  // Slide 3.5: Целевые количественные метрики качества
  {
  ...
  // Slide 4: Модульность: структура директорий
  {
  ...
  // Slide 5: Модульность: слабая связанность слоев
  {
    const s = cs(pres, "Изоляция слоев и слабая связанность");
    card(s, pres, MX, CY, 4.4, CH, C.ICE);
    s.addText("Нарушение связанности (Плохой подход)", { x: MX + 0.18, y: CY + 0.12, w: 4.04, h: 0.40,
      color: C.RED, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText("• Роутер напрямую делает SQL-запрос `SELECT * FROM tickets`.\n• Смешивание логики авторизации и рендеринга.\n• Фронтенд обращается к базе данных напрямую.\n• Любые изменения в СУБД ломают клиентский код.\n• Сложно писать изолированные тесты, так как все связано.", {
      x: MX + 0.18, y: CY + 0.62, w: 4.04, h: 3.2, color: C.DARK, fontSize: 11, fontFace: "Calibri" });

    card(s, pres, MX + 4.7, CY, 4.46, CH, C.GRNL);
    s.addText("Разделение слоев (Хороший подход)", { x: MX + 4.88, y: CY + 0.12, w: 4.1, h: 0.40,
      color: C.GREEN, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText("• Контроллер лишь принимает запрос и отдает ответ.\n• Бизнес-логика живет в `TicketService`.\n• Работа с Postgres идет через `Drizzle` репозиторий.\n• Интеграция с 1С асинхронна через outbox-очередь.\n• Любой слой можно легко заменить или протестировать.", {
      x: MX + 4.88, y: CY + 0.62, w: 4.1, h: 3.2, color: C.DARK, fontSize: 11, fontFace: "Calibri" });
  }

  // Slide 6: Тестируемость: стратегии мокирования
  {
    const s = cs(pres, "Тестируемость: стратегии мокирования (Mocking)");
    card(s, pres, MX, CY, 4.4, CH, C.ICE);
    s.addText("Изоляция базы данных", { x: MX + 0.18, y: CY + 0.12, w: 4.04, h: 0.40,
      color: C.NAVY, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText("Для тестирования роутов API реальная СУБД PostgreSQL подменяется мок-пулом `pg-mock`. Это исключает загрязнение тестовой базы данных и обеспечивает скорость выполнения тестов (миллисекунды вместо минут).", {
      x: MX + 0.18, y: CY + 0.62, w: 4.04, h: 3.2, color: C.DARK, fontSize: 11, fontFace: "Calibri" });

    card(s, pres, MX + 4.7, CY, 4.46, CH, C.GRNL);
    s.addText("Имитация внешних систем (1С, MinIO)", { x: MX + 4.88, y: CY + 0.12, w: 4.1, h: 0.40,
      color: C.NAVY, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText("• Вместо вызовов к боевому серверу 1С используется легковесный мок-сервер API.\n• Пакет AWS SDK S3 настраивается на локальную заглушку, которая не загружает файлы в облако, а симулирует сохранение в оперативной памяти.", {
      x: MX + 4.88, y: CY + 0.62, w: 4.1, h: 3.2, color: C.DARK, fontSize: 11, fontFace: "Calibri" });
  }

  // Slide 7: Поддерживаемость: строгая типизация TypeScript
  {
  ...
  // Slide 8: Поддерживаемость: Zod-валидация схем
  {
  ...
  // Slide 9: Расширяемость: паттерны проектирования
  {
    const s = cs(pres, "Расширяемость: плагины и паттерны");
    card(s, pres, MX, CY, 4.4, CH, C.ICE);
    s.addText("Паттерн Стратегия (Strategy)", { x: MX + 0.18, y: CY + 0.12, w: 4.04, h: 0.40,
      color: C.NAVY, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText("Для отправки уведомлений используется паттерн 'Стратегия'. Это позволяет легко добавить новый канал (например, Telegram, E-mail, Push) без изменения логики создания тикета. Достаточно зарегистрировать класс, реализующий интерфейс `INotificationProvider`.", { x: MX + 0.18, y: CY + 0.62, w: 4.04, h: 3.2,
      color: C.DARK, fontSize: 11, fontFace: "Calibri" });

    card(s, pres, MX + 4.7, CY, 4.46, CH, C.GRNL);
    s.addText("Слабая связанность хранилищ файлов", { x: MX + 4.88, y: CY + 0.12, w: 4.1, h: 0.40,
      color: C.NAVY, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText("Интеграция с S3 скрыта за абстракцией `IFileStorage`. В процессе разработки мы используем локальный MinIO, но при развертывании в облаке можем переключиться на AWS S3 или Yandex Object Storage простым изменением переменной среды в `.env`.", { x: MX + 4.88, y: CY + 0.62, w: 4.1, h: 3.2,
      color: C.DARK, fontSize: 11, fontFace: "Calibri" });
  }

  // Slide 10: Безопасность: авторизация JWT и SMS OTP
  {
    const s = cs(pres, "Безопасность: авторизация JWT + SMS OTP");
    card(s, pres, MX, CY, 4.4, CH, C.ICE);
    s.addText("Двухфакторный вход по SMS", { x: MX + 0.18, y: CY + 0.12, w: 4.04, h: 0.40,
      color: C.NAVY, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText("Пользователь вводит номер телефона, система генерирует случайный 6-значный код и отправляет по SMS. Код хэшируется в БД с ограничением жизни в 5 минут. При успешном вводе генерируется пара токенов.", { x: MX + 0.18, y: CY + 0.62, w: 4.04, h: 3.2,
      color: C.DARK, fontSize: 11, fontFace: "Calibri" });

    card(s, pres, MX + 4.7, CY, 4.46, CH, C.GRNL);
    s.addText("Bearer JWT (Access & Refresh)", { x: MX + 4.88, y: CY + 0.12, w: 4.1, h: 0.40,
      color: C.NAVY, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText("• Access Token: живет 15 минут, хранится в памяти, используется для авторизации запросов.\n• Refresh Token: живет 7 дней, хранится в Cookie HTTPOnly, используется для безопасного обновления access-токена.\n• Это исключает кражу токенов через XSS-скрипты.", { x: MX + 4.88, y: CY + 0.62, w: 4.1, h: 3.2,
      color: C.DARK, fontSize: 11, fontFace: "Calibri" });
  }

  // Slide 11: Безопасность: защита от сетевых атак
  {
    const s = cs(pres, "Безопасность: защита от сетевых атак");
    card(s, pres, MX, CY, 4.4, CH, C.ICE);
    s.addText("Защита данных и S3", { x: MX + 0.18, y: CY + 0.12, w: 4.04, h: 0.40,
      color: C.NAVY, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText("Файлы клиентов не хранятся в публичном доступе. Ссылка на скачивание имеет срок жизни 15 минут. На сервере включена библиотека `helmet` для защиты заголовков HTTP и CORS для блокировки посторонних доменов.", { x: MX + 0.18, y: CY + 0.62, w: 4.04, h: 3.2,
      color: C.DARK, fontSize: 11, fontFace: "Calibri" });

    card(s, pres, MX + 4.7, CY, 4.46, CH, C.GRNL);
    s.addText("Валидация входящего трафика", { x: MX + 4.88, y: CY + 0.12, w: 4.1, h: 0.40,
      color: C.NAVY, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText("• Полная очистка входящих строк от HTML-тегов (XSS Clean).\n• Параметризованные SQL-запросы в Drizzle полностью исключают SQL-инъекции.\n• Ограничение частоты запросов (Rate Limiter) спасает авторизацию и отправку SMS от перебора (Brute Force).", { x: MX + 4.88, y: CY + 0.62, w: 4.1, h: 3.2,
      color: C.DARK, fontSize: 11, fontFace: "Calibri" });
  }

  // Slide 12: Оптимистичная блокировка (Optimistic Locking)
  {
    const s = cs(pres, "Многопользовательский доступ: Optimistic Locking");
    card(s, pres, MX, CY, 4.4, CH, C.ICE);
    s.addText("Зачем нужна блокировка?", { x: MX + 0.18, y: CY + 0.12, w: 4.04, h: 0.40,
      color: C.NAVY, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText("Если Клиент и Менеджер откроют один и тот же тикет одновременно, сделают разные правки и сохранят, то последнее сохранение перезапишет первое (проблема 'lost update'). Оптимистичная блокировка решает это без блокирования таблиц БД.", { x: MX + 0.18, y: CY + 0.62, w: 4.04, h: 3.2,
      color: C.DARK, fontSize: 11, fontFace: "Calibri" });

    card(s, pres, MX + 4.7, CY, 4.46, CH, C.GRNL);
    s.addText("Механизм версии (version)", { x: MX + 4.88, y: CY + 0.12, w: 4.1, h: 0.40,
      color: C.NAVY, fontSize: 13, bold: true, fontFace: "Cambria" });
    s.addText("• Каждая запись тикета содержит поле `version` (например, 5).\n• При PUT-запросе бэкенд проверяет: `UPDATE tickets SET title = ?, version = version + 1 WHERE id = ? AND version = 5`.\n• Если версия в базе изменилась, запрос вернет 0 измененных строк. Система выдаст конфликт 409.", { x: MX + 4.88, y: CY + 0.62, w: 4.1, h: 3.2,
      color: C.DARK, fontSize: 11, fontFace: "Calibri" });
  }

  // Slide 13: Инструменты статического и динамического анализа
  {
